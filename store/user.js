...
export const useUserStore = defineStore("user", {
  ...
  actions: {
    ...
    saveTokenAndUsernameInCookie(token, username) {
      document.cookie = `userToken=${token};  path=/; max-age=${
        60 * 60 * 24 * 100
      }`;
      document.cookie = `usernameUser=${username};  path=/; max-age=${
        60 * 60 * 24 * 100
      }`;
    },
    ...
    loadTokenAndUsernameFromCookie() {
      ...
      const username = cookies
        .find((row) => row.startsWith("usernameUser="))
        ?.split("=")[1];
    ...
    logout() {
      ...
      document.cookie =
        "userToken=; username=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    },
  },
});
